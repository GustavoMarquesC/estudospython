...
- Métodos (funções) -> Representa os comportamentos do objeto. Ou seja, as ações que este objeto pode realizar no seu
sistema.
...
# O método dunder init __init__ é um método especial chamado de construtor e sua função é construir o objeto apartir da
classe
...
ATENÇÂO! Por mais que possamos criar nossas próprias funções utilizando dunder não é aconselhado. Python possui vários
métodos com esta forma de nomenclatura e pode ser que mudemos o comportamento dessas funções mágicas internas da
linguagem. Então, evite ao máximo. De preferências nunca o faça.

# Métodos são escritos em letras minúsculas. Se o nome for composto, o nome terá as palavras separas por underline

p1 = Produto('PlayStation 4', 'Vidio Game', 2300)
...
user.conta_usuarios()  # Possíovel, mas incorreta
